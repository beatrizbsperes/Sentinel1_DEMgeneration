...
def perpendicular_baseline(interferogram):
    product = ProductIO.readProduct(interferogram)
    metadata = product.getMetadataRoot()
    ...
    while stack:
        ...
        for i in range(element.getNumAttributes()):
            attr = element.getAttributeAt(i)
            name = attr.getName().lower()
            
            if 'perp baseline' in name:
                value = float(attr.getData().getElemString())
                if abs(value) > 1:  
                    result = value
        
        for i in range(element.getNumElements()):
            stack.append(element.getElementAt(i))
    result = abs(result)    
    return print(f"Perpendicular Baseline: {result} m")
...
def topsar_split(product, subswath, polarization, firstburst, lastburst):
    parameters = HashMap()
    print('Apply TOPSAR Split...')
    parameters.put('subswath', subswath)
    parameters.put('firstBurstIndex', firstburst) 
    parameters.put('lastBurstIndex', lastburst) 
...
def subset(product, aoi):
    parameters = HashMap()
    print("Subsetting...")
    parameters.put('geoRegion', aoi)
    parameters.put('sourceBands', 'elevation')
    parameters.put('bandNames', 'elevation')
